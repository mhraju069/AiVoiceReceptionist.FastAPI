            ...
            async for openai_message in openai_ws:
                ...
                # Handle tool calls
                    ...
                    if func_name == "transfer_call":
                        # Real-transfer closure: has access to call_sid via enclosing scope
                        async def _redirect_call(target_name: str):
                            phone = FORWARD_MAP.get(target_name, "")
                            if not phone:
                                raise ValueError(f"No phone number configured for target '{target_name}'")
                            host = os.getenv("PUBLIC_HOST", "")
                            redirect_url = f"https://{host}/api/twilio/forward-call?to={phone}&attempt=1"
                            update_url = f"https://api.twilio.com/2010-04-01/Accounts/{TWILIO_SID}/Calls/{call_sid}.json"
                        ...
                        result = await handle_transfer_call(
                            args, openai_ws, call_done, call_id, _log_adapter,
                            redirect_fn=_redirect_call if call_sid else None
                        )
                        await openai_ws.send(json.dumps({
