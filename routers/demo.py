                ...
                async for openai_message in openai_ws:
                    ...
                    if openai_data.get("type") == "response.audio.delta":
                        ...
                        if func_name == "book_appointment":
                            import httpx
                            # Add default calendar_id if missing since we need one for GHL
                            ...
                            try:
                                async with httpx.AsyncClient() as client:
                                    ...
                                    if resp.status_code >= 400:
                                        await _debug("tool_error", f"🔴 Booking API Error: {result}")
                                        result = {"status": "error", "message": "Failed to book appointment. Please try again later."}
                                    else:
                                        await _debug("tool_result", f"✅ Booking result: {result.get('status', 'unknown')}")
                            except Exception as e:
                                result = {"status": "error", "message": str(e)}
                                await _debug("tool_error", f"🔴 Booking failed: {e}")
